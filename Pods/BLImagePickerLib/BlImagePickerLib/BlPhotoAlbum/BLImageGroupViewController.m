...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = NSLocalizedString(@"AllAlbum", nil);
    [[BLImageHelper shareImageHelper].groupArr removeAllObjects];
    [[BLImageHelper shareImageHelper] getAllImagesGroup];
    self.navigationItem.rightBarButtonItem =[[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Cancel", nil) style:UIBarButtonItemStyleDone target:self action:@selector(clickCancel:)];
