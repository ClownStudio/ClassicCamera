...
@interface BLImageClipingViewController ()
...
@implementation BLImageClipingViewController
...
- (id)initWithImage:(UIImage *)originalImage cropFrame:(CGSize)cropFrame limitScaleRatio:(NSInteger)limitRatio {
    self = [super init];
    if (self) {
        self.cropSize = cropFrame;
        [self rechangeCropFrame];
        self.limitRatio = limitRatio;
        self.originalImage = [self fixOrientation:originalImage];
    }
    return self;
}
-(void)rechangeCropFrame{
    CGFloat x ;
    CGFloat y ;
    CGFloat w ;
    CGFloat h ;
    CGFloat max_w = SCREEN_WIDTH -30;
    CGFloat max_h = SCREEN_HEIGHT -64 -40;
    if (self.isPadPortrait || self.isPhonePortrait) {
        
        
        if ((max_w < self.cropSize.width)) {
            w = max_w;
            x = 15;
        }else{
            w = self.cropSize.width;
            x = (max_w - w)/2+15;
        }
        h = w* self.cropSize.height/self.cropSize.width;
        y = (max_h - h)/2+64+20;
        
        if (h>max_h) {
            h = max_h;
            y = 20+64;
            w = h*self.cropSize.width/self.cropSize.height;
            x = (max_w - w)/2+15;
        }
    }else{
        if ( max_h < self.cropSize.height) {
            h = max_h;
            y = 20+64;
        }else{
            h = self.cropSize.height;
            y = (max_h - h)/2+64+20;
        }
        w = h*self.cropSize.width/self.cropSize.height;
        x = (max_w - w)/2+15;
...
- (void)viewDidLoad
{
    ...
    self.title = NSLocalizedString(@"ImageClip", nil);
}
...
//    self.showImgView.frame = CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
...
- (void)initControlBtn {
    ...
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc]initWithImage:[UIImage imageNamed:@"nav_ic_back"] style:UIBarButtonItemStylePlain target:self action:@selector(backNavEvent)];
...
//    if (self.delegate && [self.delegate conformsToProtocol:@protocol(BLImageClipingViewControllerDelegate)]) {
//        [self.delegate imageClipingDidCancel:self];
//    }
//}
...
- (void)confirm:(id)sender {
    ...
    if (self.delegate && [self.delegate conformsToProtocol:@protocol(BLImageClipingViewControllerDelegate)]) {
        [self.delegate imageCliping:self didFinished:[self getSubImage]];
    }
    [self.navigationController popViewControllerAnimated:YES];
}
