...
-(void)getAllCollectionData:(NSDictionary *)dict{
    ...
    if ([title isEqualToString:@"Camera Roll"] || [title isEqualToString:NSLocalizedString(@"AlbumTitle", nil)]){
        //把相机胶卷 相册放在第一行
        self.isAllphoto = YES;
    }else{
        self.isAllphoto = NO;
...
- (void)getAllImagesGroup {
    ...
    for (PHAssetCollection *colllection  in kindAlbum) {
        ...
        if ([kindName isEqualToString:@"Camera Roll"] || [kindName isEqualToString:NSLocalizedString(@"AlbumTitle", nil)]) {
            //把相机胶卷 相册放在第一行
