...
-(void)initBaseData{
    ...
    [BLImageHelper shareImageHelper].currentGroupTitle = NSLocalizedString(@"AlbumTitle", nil);
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        [[PHImageManager defaultManager] requestImageForAsset:asset targetSize:PHImageManagerMaximumSize contentMode:PHImageContentModeAspectFill options:options resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                BLImageClipingViewController *vc = [[BLImageClipingViewController alloc]initWithImage:result cropFrame:result.size.width > result.size.height ? CGSizeMake(1920, 1080):CGSizeMake(1080, 1920) limitScaleRatio:5];
...
-(void)exitImagePickerLib:(NSArray*)selectAry assetArrt:(NSArray*)assetArry editedImage:(UIImage *)image{
    if (isCancel) {
        _cancleBlock(NSLocalizedString(@"Cancel", nil));
    }else{
...
- (void)createNav1{
    ...
    [leftBtn setTitle:NSLocalizedString(@"AllAlbum", nil) forState:UIControlStateNormal];
    ...
    self.navigationItem.rightBarButtonItem =[[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Cancel", nil) style:UIBarButtonItemStylePlain target:self action:@selector(rightBtnEvent)];
